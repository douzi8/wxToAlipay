...
module.exports =  function ({ types: t }) {
  ...
  function getSystemInfoSync (path) {
    let parentPath = path.parentPath

    if (
        parentPath.isCallExpression() &&
        parentPath.get('callee').isMemberExpression() &&
        parentPath.get('callee.property.name').node === 'getSystemInfoSync'
      ) return

    path.replaceWith(helpers.getSystemInfoSync(path.node))
  }
      ...
      CallExpression (path) {
        ...
        if (caller.isMemberExpression() && caller.get('object.name').node == 'wx') {
          ...
          if ([
            ...
            showModal(path)
          } else if (apiName === 'getSystemInfoSync') {
            getSystemInfoSync(path)
          }
        }
