...
function request (node) {
  ...
  if (properties) {
    ...
    console.log(node.args[0].parent_scope)
  //  console.log(node.args[0] instanceof U2.AST_SymbolRef)
    /*let options = node.args[0].name
    prepend = `${options}.headers = ${options}.header;`*/
    dds.forEach(item => {
      if (item === node.args[0]) {
        console.log('xxx222')
      }
  ...
  return prepend + node.print_to_string({ beautify: true })
}
